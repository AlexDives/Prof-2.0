    ...
    internal partial class SharpUpdateAcceptForm : MetroFramework.Forms.MetroForm
    {
        ...
        internal SharpUpdateAcceptForm(SharpUpdateLocalAppInfo applicationInfo, SharpUpdateXml updateInfo, int num_cur_update, int num_total_update)
        {
            ...
            this.lblNewVersion.Text = updateInfo.Tag != JobType.REMOVE ? 
                string.Format(updateInfo.Tag == JobType.UPDATE ? "Обновление: {0}\nНовая версия: {1}" : "Новая: {0}\nВерсия: {1}", Path.GetFileName(this.applicationInfo.ApplicationPath), this.updateInfo.Version.ToString()) : 
                string.Format("Удалено: {0}", Path.GetFileName(this.applicationInfo.ApplicationPath));
        }
        ...
        {
            new SharpUpdateInfoForm(this.applicationInfo, this.updateInfo).ShowDialog();
        }
    }
}
