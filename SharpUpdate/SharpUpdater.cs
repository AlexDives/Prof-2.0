    ...
    public class SharpUpdater
    {
        ...
        private void BgWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
			...
			if (!e.Cancelled)
			{
				...
				if (JobsFromXML != null)
				{
                    Console.WriteLine("Номер обновления " + JobsFromXML.Length);
                    ...
                    if (showMsgBox)
                    {
                        //Если обновлён до последней 
                        //MessageBoxEx.Show(ParentForm, "Версия приложения является последней");
                    }
                    else
                    {
                        if (acceptJobs > 0)
        ...
        /// ОШИБКА НАЧИНАЕТСЯ ОТ СЮДА RESULT = NO
        private void DownloadUpdate(SharpUpdateXml update, SharpUpdateLocalAppInfo applicationInfo)
        {
            ...
            {
                MessageBoxEx.Show(ParentForm, "Загрузка обновления была отменена.\n Эта программа не была изменена.", "Обновление отменено", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBoxEx.Show(ParentForm, "При загрузке обновления возникла проблема.\n Пожалуйста, попробуйте позже", "Ошибка загрузки обновления", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        {
            MessageBoxEx.Show(ParentForm, "Приложение перезапуститься через 3 секунды", "Успешно", 3000);
            UpdateApplications();
            Application.Exit();
        }
        ...
        private void UpdateApplications()
        {
            ...
            if (curAppidx > -1)
            {
                ...
                Console.WriteLine("Обновите и запустите главное приложение: " + argument_complete);
            }
            else
            {
                argument_complete = string.Format(argument_start, Path.GetDirectoryName(ParentPath), Path.GetFileName(ParentPath));
                Console.WriteLine("Запустить главное приложение: " + argument_complete);
            }
